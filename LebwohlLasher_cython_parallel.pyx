...
cdef double[:,::1] initdat(int nmax):
    ...
    """
    cdef double[:,::1] arr = np.random.random_sample((nmax,nmax))*2.0*pi
    return arr
#=======================================================================
...
#     FileOut.close()
#=======================================================================
cdef float one_energy(double[:,::1] arr, int ix,int iy,int nmax):
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
...
#
    ...
    return en
#=======================================================================
cdef float all_energy(double[:,::1] arr,int nmax, int thread):
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
      nmax (int) = side length of square lattice.
    ...
    for i in prange(nmax, nogil = True, num_threads = thread):
        for j in range(nmax):
            enall += one_energy(arr,i,j,nmax)
    return enall
#=======================================================================
cdef double get_order(double[:,::1] arr, int nmax, int thread):
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
      nmax (int) = side length of square lattice.
...
#=======================================================================
cdef float MC_step(double[:,::1] arr, float Ts, int nmax):
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
	  Ts (float) = reduced temperature (range 0 to 2);
...
#=======================================================================
def main(program, int nsteps, int nmax, float temp, pflag, int thread):
    """
    Arguments:
    ...
    energy[0] = all_energy(lattice,nmax,thread)
    ratio[0] = 0.5 # ideal value
    order[0] = get_order(lattice,nmax,thread)

    # Begin doing and timing some MC steps.
    initial = time.time()
    for it in range(1,nsteps+1):
        ratio[it] = MC_step(lattice,temp,nmax)
        energy[it] = all_energy(lattice,nmax,thread)
        order[it] = get_order(lattice,nmax,thread)
    final = time.time()
...
if __name__ == '__main__':
    if int(len(sys.argv)) == 6:
        ...
        main(PROGNAME, ITERATIONS, SIZE, TEMPERATURE, PLOTFLAG, THREAD)
    else:
