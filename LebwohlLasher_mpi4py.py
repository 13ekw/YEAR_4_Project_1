...
def MC_step(arr,Ts,nmax):
    ...
    end_of_chunk = start_of_chunk + chunk


    for i in range(start_of_chunk, end_of_chunk):
        for j in range(nmax):
            ix = xran[i,j]
            iy = yran[i,j]
            ang = aran[i,j]
...
def main(program, nsteps, nmax, temp, pflag):
    ...
    if (rank == 0):
      ...
      if ((size - 1) > nmax) or ((size - 1) < 1):
        print(f"Please use a cpus_per_task between 1 and {nmax}")
